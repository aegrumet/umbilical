...
Keys are used to verify the Umbilical signature, which is passed in the request
as follows:
...
The Umbilical signature has following format:

```
    t=<timestamp in milliseconds>,s=<hmac>
```

The hmac is generated by concatenating the timestamp, a requestString
...
The requestString is the request URL without the protocol (https:// or wss://), and including the query string for http(s) only.
